...
export async function getReservasPendentes(): Promise<number> {
  ...
  try {
    ...
    console.error("getReservasPendentes:", error);
    return 0;
  }
}
...
export async function getTopCategorias(
  ...
  try {
    ...
    const rows = await sql<{ categoria: string; percentagem: string }[]>`
      SELECT
        ...
        ROUND(
          ...
          / NULLIF((
            ...
            WHERE o2.created_at >= ${desde.toISOString()}
      ...
      WHERE o.created_at >= ${desde.toISOString()}
    ...
    return rows.map((r) => ({
      // garante 1ª letra maiúscula (e o resto “certinho”)
      categoria: r.categoria
        ? r.categoria.charAt(0).toUpperCase() +
          r.categoria.slice(1).toLowerCase()
        : "",
