// TODO: Descomentar quando criar a tabela feedback e configurar R2
...
export async function createFeedback(formData: FormData) {
  try {
    ...
    if (!comment || comment.trim().length < 10) {
      return { success: false, error: "Comentário muito curto (mínimo 10 caracteres)" };
    }
    ...
    if (imageFile && imageFile.size > 0) {
      ...
      if (!imageFile.type.startsWith("image/")) {
        return { success: false, error: "Ficheiro inválido (apenas imagens)" };
      }
