...
export default function ReservationForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setStatus("error");
      setErrorMsg("Falha de rede. Verifica a ligação e tente novamente");
    }
  }
    ...
    <form onSubmit={handleSubmit} className="grid gap-4">
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="grid gap-2">
          <span className="text-sm font-semibold text-[#1E3A8A]/80">Nome</span>
          <input
            required
            value={form.name}
        ...
        <label className="grid gap-2">
          <span className="text-sm font-semibold text-[#1E3A8A]/80">Email</span>
          <input
            required
            type="email"
            value={form.email}
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <label className="grid gap-2">
          <span className="text-sm font-semibold text-[#1E3A8A]/80">Data</span>
          <input
            required
            type="date"
            value={form.date}
        ...
        <label className="grid gap-2">
          <span className="text-sm font-semibold text-[#1E3A8A]/80">Hora</span>
          <input
            required
            type="time"
            value={form.time}
