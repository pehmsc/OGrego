...
export function SignupForm({
  ...
  async function onVerify(e: React.FormEvent) {
    ...
    try {
      ...
      setError(
        "Não deu para concluir a verificação. Confirma o código e tenta outra vez.",
      );
    } catch (err: any) {
      setError(getClerkError(err));
    } finally {
  ...
  async function signUpWithProvider(provider: "oauth_google" | "oauth_apple") {
    if (!isLoaded) return;
          ...
          {!needsVerification ? (
            <form onSubmit={onSubmit}>
              <FieldGroup>
                ...
                <Field>
                  {/* ✅ Smart CAPTCHA mount point (reserva um bocadinho de espaço) */}
                  <div
                    id="clerk-captcha"
                  ...
                  <Field className="grid grid-cols-2 gap-4">
                    <Button
                      ...
                      type="button"
                      disabled={loading}
                      onClick={() => signUpWithProvider("oauth_apple")}
                    >
                      <svg
                      ...
                      <span className="sr-only">Registar com Apple</span>
                    </Button>
            ...
            <form onSubmit={onVerify}>
              <FieldGroup>
                <Field>
                  ...
                  />
                  <FieldDescription>
                    Enviámos um código para o teu email. Mete-o aqui e
                    acabou-se.
                  </FieldDescription>
                </Field>
                ...
                <Field>
                  {/* (Opcional) Se o Clerk decidir usar CAPTCHA também aqui, já tem onde montar */}
                  <div
                    id="clerk-captcha"
                    className="mt-2 min-h-[44px] w-full"
                  />
